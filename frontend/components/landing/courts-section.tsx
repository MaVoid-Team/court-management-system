...
export function CourtsSection() {
    ...
    useEffect(() => {
        if (loading) return; // Wait to animate until loaded or error
    ...
    return (
        <section ref={root} id="courts" className="relative w-full py-24 overflow-hidden bg-muted/10 border-t border-border/40">
            <div className="relative z-10 w-full px-8 md:px-16 lg:px-24">
                ...
                {loading ? (
                    ...
                    <div className="p-8 border border-destructive/20 bg-destructive/5 rounded-2xl text-center">
                        <p className="text-destructive font-medium mb-2">Failed to load courts.</p>
                        <Button variant="outline" onClick={() => fetchPublicCourts()}>Try again</Button>
                    </div>
                ) : courts.length === 0 ? (
                    <div className="p-12 border border-border/50 bg-card rounded-2xl text-center">
                        <p className="text-muted-foreground text-lg">No courts available at the moment.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {courts.map((court) => (
                            ...
                            >
                                <div>
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <Badge variant={court.active ? "outline" : "secondary"} className="bg-background">
                                            {court.active ? "Available" : "Maintenance"}
                                        </Badge>
                                    </div>
                                    <div className="mb-6">
                                        <span className="text-3xl font-black">{formatCurrency(Number(court.price_per_hour))}</span>
                                        <span className="text-muted-foreground text-sm"> / sq/hr</span>
                                    </div>
                                </div>
                                <div className="pt-4 border-t border-border/40">
                                    {court.active ? (
                                        <Button asChild className="w-full font-semibold group-hover:gap-2 transition-all">
